...
module.exports.doExecuteCommand = function (_params) {
    var deferred = q.defer();
    var req = constructRunCommandRequest(_params);

    winrm_http_req.sendHttp(req, _params.host, _params.port, _params.path, _params.auth).then(function (result) {
        if (result['s:Envelope']['s:Body'][0]['s:Fault']) {
            deferred.reject(result['s:Envelope']['s:Body'][0]['s:Fault'][0]['s:Code'][0]['s:Subcode'][0]['s:Value'][0]);
        } else {
            var commandId = result['s:Envelope']['s:Body'][0]['rsp:CommandResponse'][0]['rsp:CommandId'][0];
            deferred.resolve(commandId);
        }
    });
...
module.exports.doExecutePowershell = function (_params) {
    _params['command'] = generatePowershellCommand(_params);
    return this.doExecuteCommand(_params);
};

module.exports.doReceiveOutput = function (_params) {
    var deferred = q.defer();
    ...
    winrm_http_req.sendHttp(req, _params.host, _params.port, _params.path, _params.auth).then(function (result) {
        if (result['s:Envelope']['s:Body'][0]['s:Fault']) {
            ...
            if (successOutput) {
                deferred.resolve(successOutput.trim());
                return;
            }
            deferred.resolve(failedOutput.trim());
        }
    }).catch(function (err) {
        deferred.reject(err);
    });
